...
| args => do
  ...
  let var1 ← genId
  let var2 ← genId
  let rest ← pfCombine (← read) (args.getArgs.toList|>.drop 1|>.toArray)
...
#coreFmt Lean.Parser.Command.optDeclSig fun
  | `($arguments:term $returnValue:term) => do
    ...
    let returnVal ← (format returnValue)
    let args ← (pfCombineWithSeparator (text " " <^> PPL.nl) (← read) arguments.raw.getArgs)
...
| stx => do
  ...
  if args.size == 0 then
    ...
    return (← pfCombineWithSeparator (text " ") (← read) args)


#coreFmt Lean.Parser.Term.typeSpec fun
| a => do
  pfCombineWithSeparator (text " ") (← read) a.getArgs


#coreFmt Lean.Parser.Command.definition fun
| args => do
  return PPL.nest 2 (← (combineArgs ((text "") <^> PPL.nl) args))
...
| `($docComment:term $attributes:term $visibility:term $noncomputableS:term $unsafeS:term $partialS:term) => do
  let modifiers ← combineArgs' (text " ") [attributes, visibility, noncomputableS, unsafeS, partialS]
  return (← format docComment) <> modifiers ?> text " "
| _ => failure
...
| `($docComment:term $attributes:term $visibility:term $noncomputableS:term $unsafeS:term $partialS:term) => do
-- | args => do
  -- let docComment ← r (args.get! 0)
  -- let rest := args.extract 1
  let modifiers ← combineArgs' (text " ") [attributes, visibility, noncomputableS, unsafeS, partialS]

  -- let mut modifiers ← pfCombineWithSeparator (text " ") r rest
...
#coreFmt Lean.Parser.Term.let fun (r : RuleRec) => fun
| #[letSymbol, declaration, unknown1, after] => do
  return (← r letSymbol) <> text " " <> (← r declaration) <> (← r unknown1) <> PPL.nl <> (← r after)
| _ => failure


#coreFmt Lean.Parser.Term.letIdDecl fun (r : RuleRec) => fun
| #[var, unknown1, typeInfo, assignOperator, content] => do
  ...
  return (← r var) <> text " " <> (← r unknown1) <> (followWithSpaceIfNonEmpty (← r typeInfo)) <> (← r assignOperator) <> (← nest 2 (addSpaceOrNewLine r content))
| a => do
  failure

-- TODO: figure out what the suffix is used for.

#coreFmt Lean.Parser.Termination.suffix fun (_ : RuleRec) => fun
...
#coreFmt Lean.Parser.Term.app fun (r : RuleRec) => fun
| #[functionName, arguments]  => do
  return (← r functionName) <> text " " <> (← pfCombineWithSeparator (text " ") r arguments.getArgs)
| _ => failure

#coreFmt Term.app fun (r : RuleRec) => fun
| #[functionName, arguments]  => do
  return (← r functionName) <> text " " <> (← pfCombineWithSeparator (text " ") r arguments.getArgs)
| _ => failure

def termOperator : Rule := fun (r : RuleRec) => fun
| #[left, operator, right] => do
  return (← r left) <> (PPL.nl <^> text " ") <> (← r operator) <> text " " <> (← r right)
| _ => failure
...
#coreFmt «term_+_» termOperator
#coreFmt «term_-_» termOperator
#coreFmt «term{}» fun (r : RuleRec) => fun
| args => (pfCombineWithSeparator (text "")) r args


#coreFmt Lean.Parser.Command.instance fun (r : RuleRec) => fun
| #[kind, instanceAtom, unknown1, unknown2, decl, whereStructInst] => do
  assumeMissing unknown1
  assumeMissing unknown2
  let declaration ← nest 4 do
    pfCombineWithSeparator (text " ") r #[kind, instanceAtom, decl]
  let struct ← nest 2 do
    r whereStructInst
  return declaration <> text " " <> struct
| _ => failure

#coreFmt Lean.Parser.Command.whereStructInst fun (r : RuleRec) => fun
| args => pfCombineWithSeparator PPL.nl r args

#coreFmt Lean.Parser.Term.structInstFields fun (r : RuleRec) => fun
| args => pfCombineWithSeparator PPL.nl r args

#coreFmt Lean.Parser.Term.structInstFieldDef fun (r : RuleRec) => fun
| args => nest 2 <| pfCombineWithSeparator (text " " <^> PPL.nl) r args

#coreFmt Lean.Parser.Term.fun fun (r : RuleRec) => fun
| args => pfCombineWithSeparator (text " ") r args

#coreFmt Lean.Parser.Term.structInstField fun (r : RuleRec) => fun
| args => pfCombineWithSeparator (text " ") r args

#coreFmt Lean.Parser.Term.basicFun fun (r : RuleRec) => fun
| #[args, unknown1 , arrowAtom, content] => do
  assumeMissing unknown1
  let argsFormatted ← pfCombineWithSeparator (text " ") r args.getArgs
  if isEmpty [] argsFormatted then
    failure
  else
    return argsFormatted <> text " " <> (← r arrowAtom) <> text " " <> (← r content)
| _ => failure

#coreFmt Lean.Parser.Term.typeAscription fun (r : RuleRec) => fun
| #[firstParen, vars, atom, type, lastParen] => do
  return (← r firstParen) <> (← pfCombineWithSeparator (text " ") r #[vars, atom, type]) <> (← r lastParen)
| _ => failure

#coreFmt Lean.Parser.Term.proj fun (r : RuleRec) => fun
| args => pfCombineWithSeparator (text "") r args

#coreFmt Lean.Parser.Command.declSig fun (r : RuleRec) => fun
| args => pfCombineWithSeparator (text " ") r args

#coreFmt Lean.Parser.Command.docComment fun (r : RuleRec) => fun
| args => return (← pfCombineWithSeparator (PPL.nl) r args) <> PPL.nl
-- #coreFmt Lean.Parser.Command.declSig fun (r : RuleRec) => fun
-- | args => pfCombineWithSeparator (text " ") r args


-- TODO: think more about choice, at the moment we just take the first option
#coreFmt choice fun (r : RuleRec) => fun
| args => r (args.get! 0)

#coreFmt Lean.Parser.Term.paren fun (r : RuleRec) => fun
| args => pfCombineWithSeparator (text "") r args

#coreFmt Lean.Parser.Command.namespace fun (r :RuleRec) => fun
| args => pfCombineWithSeparator (text " ") r args

#coreFmt Lean.Parser.Command.end fun (r :RuleRec) => fun
| args => pfCombineWithSeparator (text " ") r args


-- def test (stx : Syntax) : IO Unit=
--   let a  := TSyntax.mk stx
